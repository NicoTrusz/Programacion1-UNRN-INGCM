 ...
 * @returns Largo final de la cadena destino o código de error negativo.
 */
int insertar_seguro(char destino[], size_t capacidad, const char insertar[], size_t posicion) {
    ...
    if (largo_destino == capacidad) return CADENA_SIN_TERMINADOR;

    // Calcular largo de insertar
    while (largo_insertar < capacidad && insertar[largo_insertar] != '\0') {
        largo_insertar++;
    }
    if (largo_insertar == capacidad) return CADENA_SIN_TERMINADOR;

    // Verificar posición válida
    if (posicion > largo_destino) return ERROR_CAPACIDAD_INSUFICIENTE;

    // Verificar capacidad suficiente
    if (largo_destino + largo_insertar >= capacidad) return ERROR_CAPACIDAD_INSUFICIENTE;
    ...
    return largo_destino + largo_insertar;
}
